...
bool executeQuery(char* query) {    //funzione base che deve essere richiamata dal main
    ...
    ptr = strtok_r(tmpQuery, " ", &end_col); //splitta la stringa iniziale per capire che tipo di query deve eseguire
    ...
    if(strcmp(ptr,"CREATE") == 0) {  //CREATE TABLE (Tempo O(n colonne))
        ...
        ptr = strtok_r(NULL, " ", &end_col); //salta a TABLE
        if(strcmp(ptr, "TABLE") != 0) //controlla che sia scritto bene
            return false;
        
        ptr = strtok_r(NULL, " ", &end_col); //salta al nome della tabella
        strcpy(nome, ptr);  //lo copia in name
        nome_lenght = (int)strlen(ptr); //lunghezza stringa
        
        ptr = strtok_r(NULL, ",", &end_col); //salta alle colonne
        if(ptr == NULL)
            return false;
        ...
        while(ptr != NULL) { //per ogni colonna
            ...
            ptr = strtok_r(NULL, ",", &end_col); //passa alla colonna sucessiva
        }
        ...
        ptr = strtok_r(NULL, " ", &end_col);  //salta a INTO
        if(strcmp(ptr, "INTO") != 0)            //controlla che INTO sia scritto correttamente
            return false;
        ptr = strtok_r(NULL, " ", &end_col);  //salta al nome della tabella
        
        char *fileName = NULL, *end_control;  //fileName = nome del file da aprire, end_control = utile per strtok_r(vedi end_col)
        ...
        ptrStringa = strtok_r(stringaSplit," ", &end_control); //prima splittatura che prende table
        ptrStringa = strtok_r(NULL, " ", &end_control);     //prende il nome della tabella
        ptrStringa = strtok_r(NULL, " ", &end_control);     //passa a columns
        ptrStringa = strtok_r(NULL, ",;",&end_control);    //splitta e arriva ai nomi delle colonne
        ...
        while(ptrStringa != NULL) {     //legge tutte le colonne del file
            ...
            ptrStringa = strtok_r(NULL, ",;", &end_control); //splitta la sucessiva colonna
        }
        ...
        if(strcmp(ptr, "*") == 0) { //se SELECT *
            ...
            ptr = strtok_r(NULL, " ", &end_col); //splitta FROM
        ...
        ptr = strtok_r(NULL, " ", &end_col); //splitta per vedere quale tipo di filtro bisogna performare
        ...
        if(strcmp(ptr, "WHERE") == 0) {     //se è presente il filtro where
            ...
            ptr = strtok_r(NULL, " ", &end_col);  //controlla che BY sia scritto bene
            if(strcmp(ptr, "BY") != 0)
                return false;
            ...
            ptr = strtok_r(NULL, " ,", &end_col);  //verifica che sia presente soltanto un campo su cui fare GROUP BY
...
//FUNZIONE SELECT SENZA NESSUN FILTRO APPLICATO (Tempo O(n righe * n colonne + n colonne ^ 2))
bool selectNoFilter(char *query, char **colonne, char *tabella, int nome_lenght, int num_colonne) {    //query = query testuale, colonne = lista colonne della select (se NULL la select ha *), tabella = nome della tabella su cui si fa la select, nome_lenght = lunghezza nome tabella, num_colonne = numero di colonne della select
    ...
    for(int i = 0; i < rows; i++) { //scrive nel file ogni riga
        ...
        fseeko(file_output,-1,SEEK_END);
        position = ftello(file_output);
        ftruncate(fileno(file_output), position);  //cancella l ultima virgola
        fprintf(file_output,";\n"); //aggiunge il ;
    }
    fprintf(file_output, "\n");
...
//FUNZIONE SELECT CON FILTRO WHERE (Tempo O(n righe * n colonne + n colonne ^ 2))
    ...
    for(int i = 0; i < rows; i++) { //scrive nel file ogni riga che soddisfa la condizione
        ...
        for(int j = 0; j < columns; j++) {
            if(toPrint[j] == 2 || toPrint[j] == 3) {//campo = colonna su cui si filtra, operando = > < >= <= == !=, parametro = valore del filtro
                ...
                else    //se operando è errato non stampa nulla
                    ifOk = 0;
            }
        }
...
//FUNZIONE SELECT CON FILTRO ORDER BY (Tempo O(n righe ^ 2 + n righe * n colonne + n colonne ^ 2))
bool selectOrder(char *query, char **colonne, char *tabella, int nome_lenght, int num_colonne, char *campo, char *tipo) { //query = query testuale, colonne = lista colonne della select (se NULL la select ha *), tabella = nome della tabella su cui si fa la select, nome_lenght = lunghezza nome tabella, num_colonne = numero di colonne della select, campo = campo su cui si ordina, tipo = ASC DESC
    ...
    int colonnaDaOrd = 0;   //indice che indica la colonna su cui si deve ordinare
...
//FUNZIONE SELECT CON FILTRO GROUP BY (Tempo O(n righe * n colonne + n righe * n occorrenze))
bool selectGroup(char *query, char **colonne, char *tabella, int nome_lenght, int num_colonne, char *campo) {   //query = query testuale, colonne = lista colonne della select (se NULL la select ha *), tabella = nome della tabella su cui si fa la select, nome_lenght = lunghezza nome tabella, num_colonne = numero di colonne della select, campo = campo su cui si ordina, tipo = ASC DESC
    ...
    int colonnaDaRagg = 0;   //indice che indica la colonna su cui si deve ordinare
    while((pnt=strtok(NULL," ,;\n")) != NULL){ //legge le colonne
        if(strcmp(pnt,campo) == 0){ //continua finche non trova la colonna su cui bisogna ordinare
            break;
        }
    ...
    int ifFound = 0;    //utile a capire se la riga è gia presente in gruppi o è da aggiungere
    for(int i = 0; i < rows; i++) { //cicla tutte le righe di dati
        ifFound = 0;
